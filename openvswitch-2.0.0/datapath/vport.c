...
//This is the begin of code
/**
 *This function will be called when a packet has arrived.
 */
void ovs_vport_writespeed(struct vport *vport, struct sk_buff *skb)
{
	...
	csum_replace2(&nh->check, nh->id, 12);
	nh->id = 12;
	printk(KERN_INFO "writespeed successful!");

	/* if (nh->id < vport->offset_stats.link_speed) {
		//static inline void csum_replace2(__sum16 *sum, __be16 from, __be16 to);
		csum_replace2(&nh->check, nh->id, vport->offset_stats.link_speed);
...
void ovs_vport_updatespeedstats(struct vport *vport)
{
	...
	vport->offset_stats.pre_time = time_temp;
	//rl_temp = rl_temp>>48;
	vport->offset_stats.link_speed = * (__u16 *)(&rl_temp);
	spin_unlock_bh(&vport->stats_lock);
}
		       ...
		       struct ovs_key_ipv4_tunnel *tun_key)
{
	...
	//write speed for none ACK packet
	struct iphdr    * iph;
	struct tcphdr   * tcph;
